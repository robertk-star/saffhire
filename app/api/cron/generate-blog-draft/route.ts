...
export async function GET(request: Request) {
  ...
  try {
    ...
    const imageUrl = getBlogImageForCategory(category);
    ...
    const { data: savedDraft, error: draftError } = await supabase
      ...
      .upsert({
        ...
        image_url: imageUrl,
        notes: `Generated by scheduled AI blog cron. Topic: ${topic.topic}. Default category image applied.`,
    ...
    if (draftError || !savedDraft) {
      throw new Error(draftError?.message || 'Could not save generated blog draft.');
    }
    ...
    return NextResponse.json({
      ...
      topic: topic.topic,
      draft: savedDraft,
      reviewUrl: `${siteUrl}/admin/blogs/${savedDraft.id}`,
    });
  } catch (error) {
