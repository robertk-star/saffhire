...
const ALERT_TO = 'robertk@saffhire.com';
...
async function sendPublishEmail(post: { title: string; excerpt: string; slug: string; id: string; category: string }) {
  const apiKey = process.env.RESEND_API_KEY;
  if (!apiKey) return { sent: false, reason: 'RESEND_API_KEY missing' };

  const publicUrl = `https://saffhire.com/blog/${post.slug}`;
  const editUrl = `https://saffhire.com/admin/blogs/${post.id}`;
  const text = [
    `A new SaffHire blog post was published automatically.`,
    '',
    `Title: ${post.title}`,
    `Category: ${post.category}`,
    `Live URL: ${publicUrl}`,
    `Edit in admin: ${editUrl}`,
    '',
    post.excerpt,
    '',
    'You can edit or unpublish this post anytime from the admin blog editor.',
  ].join('\n');
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: FROM,
      to: [ALERT_TO],
      subject: `New SaffHire blog published: ${post.title}`,
      text,
    }),
  });

  const details = await response.text().catch(() => 'Unknown email error');
  return { sent: response.ok, reason: details };
}
  ...
  try {
    ...
    const { data: savedDraft, error: draftError } = await supabase
      ...
      .upsert({
        ...
        image_url: imageUrl,
        notes: `Auto-published by ${authMode === 'admin' ? 'manual admin request' : 'scheduled AI blog cron'}. Topic: ${topic.topic}. ${imageNote}`,
        status: 'published',
        approved_at: now,
        published_at: now,
      }, { onConflict: 'slug' })
      .select('id, slug, title, status, excerpt, category')
      .single();

    if (draftError || !savedDraft) throw new Error(draftError?.message || 'Could not save generated blog post.');

    const emailResult = await sendPublishEmail({
