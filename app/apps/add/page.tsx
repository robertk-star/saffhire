    ...
    <main className="min-h-screen bg-slate-50 px-6 py-10">
      <div className="mx-auto max-w-3xl">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-600">
                Add or delete links from the shared app links page. Changes are saved in Supabase and show on other devices.
              </p>
            </div>
            <a
              href="/apps"
        ...
        <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-2 text-sm text-slate-600">Select a link below and delete it from the shared list.</p>
          ...
          {result.links.length === 0 ? (
            ...
            <form action="/api/admin/app-links/delete" method="post" className="mt-5 grid gap-4">
              <label className="block">
                <span className="mb-2 block text-sm font-bold text-slate-700">Link to delete</span>
                <select name="id" className="w-full rounded-md border border-slate-300 px-4 py-3 text-sm" required>
                  {result.links.map((link) => (
                    <option key={link.id} value={link.id}>{link.category} — {link.name}</option>
                  ))}
                </select>
              </label>
              <button className="w-fit rounded-md border border-red-200 bg-red-50 px-5 py-3 text-sm font-bold text-red-700 hover:bg-red-100">
